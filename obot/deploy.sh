...
# Set dns resolution
if $(cat /etc/hosts | grep -q obot.test);then
 sudo sed -i -e "s/.*obot.test.*/$(echo "127.0.0.1 obot.test")/" /etc/hosts;
 else 
 echo "127.0.0.1 obot.test" | sudo tee -a /etc/hosts;
fi
...
kubectl create secret generic  obot-secret --from-env-file=./secrets/.env.$ENVIRONMENT --namespace $NAMESPACE
...
#kubectl port-forward svc/svc-obot 7474:7474 7687:7687 --namespace $NAMESPACE