...
# Set dns resolution
if $(cat /etc/hosts | grep -q ollama.test);then
 sudo sed -i -e "s/.*ollama.test.*/$(echo "127.0.0.1 ollama.test")/" /etc/hosts;
 else 
 echo "127.0.0.1 ollama.test" | sudo tee -a /etc/hosts;
fi
...
kubectl create secret generic  ollama-secret --from-env-file=./secrets/.env.$ENVIRONMENT --namespace $NAMESPACE
...
#kubectl port-forward svc/svc-ollama 7474:7474 7687:7687 --namespace $NAMESPACE